...
class positionSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        if x:
            ...
            position['PositionName'] = response.xpath("//div[@class='top-fixed-box']//h1/text()").extract()
            position['CompanyName'] = response.xpath("//div[@class='top-fixed-box']//h2/a/text()").extract()
            position['MonthlySalaryL'] = response.xpath("//div[@class='terminalpage-left']/ul/li[1]/strong/text()").extract()
            #position['MonthlySalaryH'] = response.xpath("//div[@class='top-fixed-box']//h2/a/text()").extract()
            position['WorkAddress'] = response.xpath("//div[@class='terminalpage-left']/ul/li[2]/strong/a/text()").extract()
            position['Degree'] = response.xpath("//div[@class='terminalpage-left']/ul/li[6]/strong/text()").extract()
            position['WorkExperience'] = response.xpath("//div[@class='terminalpage-left']/ul/li[5]/strong/text()").extract()
            position['RecruitingNumbers'] = response.xpath("//div[@class='terminalpage-left']/ul/li[7]/strong/text()").extract()
            position['PositionCategory'] = response.xpath("//div[@class='terminalpage-left']/ul/li[8]/strong/a/text()").extract()
